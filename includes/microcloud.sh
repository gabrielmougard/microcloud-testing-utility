...
enable_ovn_uplink() {
  # Enable the enp6s0 on the VMs for the UPLINK of the OVN network
  for i in $(seq 1 4); do
      lxc exec micro0${i} --project microcloud-test -- echo 0 > /proc/sys/net/ipv6/conf/enp6s0/accept_ra
      lxc exec micro0${i} --project microcloud-test -- ip link set enp6s0 up
  done
}
